...
export function AIAnalysisPage() {
 ...
 {rec.type === 'urgent' && <AlertTriangle className="w-4 h-4 text-[var(--destructive)]" />}
 {rec.type === 'opportunity' && <Target className="w-4 h-4 text-[var(--primary)]" />}
 {rec.type === 'cost' && <DollarSign className="w-4 h-4 text-[var(--secondary)]" />}
 </div>
 <div className="flex-1">
 <h5 className="font-medium text-sm mb-1">{rec.title}</h5>
 <p className="text-xs text-[var(--primary)] mb-2">{rec.impact}</p>
