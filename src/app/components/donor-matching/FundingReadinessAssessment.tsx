...
const docStatusConfig: Record<string, { icon: React.ElementType; color: string; label: string }> = {
 ...
 expired: { icon: AlertTriangle, color: 'text-[var(--warning)]', label: 'منتهي الصلاحية' },
 partial: { icon: AlertCircle, color: 'text-[var(--secondary)]', label: 'جزئي' },
};
...
export function FundingReadinessAssessment({ onNavigate }: FundingReadinessAssessmentProps) {
 ...
 const scoreColor = overallScore >= 85 ? 'text-[var(--primary)]' : overallScore >= 70 ? 'text-[var(--warning)]' : 'text-red-500';
 const scoreBarColor = overallScore >= 85 ? 'bg-[var(--primary)]/[0.08]0' : overallScore >= 70 ? 'bg-amber-400' : 'bg-red-400';
 ...
 <div className={`h-full rounded-full ${d.score >= 85 ? 'bg-[var(--primary)]/[0.08]0' : d.score >= 70 ? 'bg-amber-400' : 'bg-red-400'}`} style={{ width: `${d.score}%` }} />
 </div>
 ...
 <div key={req.id} className={`p-3 rounded-xl border ${req.priority === 'high' ? 'border-[var(--destructive)]/[0.3] bg-[var(--destructive)]/[0.08]' : 'border-[var(--warning)]/[0.3] bg-[var(--warning)]/[0.08]'}`}>
 ...
 <span className={`text-xs px-2 py-0.5 rounded-full ${req.priority === 'high' ? 'bg-[var(--destructive)]/[0.1] text-[var(--destructive)]' : 'bg-amber-100 text-[var(--warning)]'}`}>
 ...
 <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 z-10 ${stage.status === 'done' ? 'bg-[var(--primary)]/[0.08]0' : stage.status === 'current' ? 'bg-primary' : 'bg-muted border-2 border-border'}`}>
 {stage.status === 'done' ? <CheckCircle2 className="w-5 h-5 text-[var(--primary-foreground)]" /> : stage.status === 'current' ? <RefreshCw className="w-4 h-4 text-primary-foreground animate-spin" /> : <Clock className="w-4 h-4 text-muted-foreground" />}
 </div>
 </div>
 ))}
 </div>
