...
export function SubmissionPreparation({ donorId, onNavigate }: SubmissionPreparationProps) {
 ...
 ready: 'text-[var(--primary)] bg-[var(--primary)]/[0.08]',
 missing: 'text-red-500 bg-[var(--destructive)]/[0.08]',
 expired: 'text-[var(--warning)] bg-[var(--warning)]/[0.08]',
 };
 ...
 {/* Donor Info */}
 <div className="bg-gradient-to-l from-indigo-600 to-violet-700 rounded-2xl p-5 text-[var(--primary-foreground)]">
 ...
 <div className={`h-full rounded-full transition-all ${pct >= 80 ? 'bg-[var(--primary)]/[0.08]0' : pct >= 60 ? 'bg-amber-400' : 'bg-red-400'}`} style={{ width: `${pct}%` }} />
 </div>
 <div className="flex flex-wrap gap-2 justify-end">
 {readinessSteps.map((step, i) => (
 <div key={i} className={`flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-full border ${step.done ? 'bg-[var(--primary)]/[0.08] border-[var(--primary)]/[0.3] text-[var(--primary)]' : 'bg-muted border-border text-muted-foreground'}`}>
 ...
 <div className="mt-3 p-2.5 rounded-lg bg-[var(--warning)]/[0.08] border border-[var(--warning)]/[0.3] text-right">
 ...
 {checklist.map(item => (
 <div key={item.id} className={`flex items-start gap-3 p-3 rounded-lg border transition-colors ${item.checked ? 'border-[var(--primary)]/[0.3] bg-[var(--primary)]/[0.08]' : 'border-border hover:bg-muted/30'}`}>
 ...
 <button
 className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-l from-indigo-600 to-violet-600 text-[var(--primary-foreground)] rounded-xl hover:opacity-90 transition-opacity shadow-lg"
 >
 <Package className="w-4 h-4" /> توليد حزمة التقديم الكاملة
 </button>
 <button
 onClick={() => setSubmittedForApproval(true)}
 className={`flex items-center gap-2 px-5 py-2.5 rounded-xl border transition-colors ${submittedForApproval ? 'bg-[var(--primary)]/[0.08] border-[var(--primary)]/[0.3] text-[var(--primary)]' : 'border-border text-foreground hover:bg-muted'}`}
 >
 ...
 <button
 disabled={pct < 80}
 className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-[var(--primary)]/[0.3] text-[var(--primary)] bg-[var(--primary)]/[0.08] hover:bg-[var(--primary)]/[0.1] transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
 >
