...
export function ProjectCollaborationModule() {
 ...
 <div className="bg-card rounded-xl p-6 border border-[var(--destructive)]/[0.3] shadow-sm">
 <div className="flex items-center justify-between mb-2">
...
function ChatView({
 ...
 <p className="text-[var(--destructive)] text-sm mb-2">{conversationsError}</p>
 <button
 onClick={() => refetchConversations()}
 className="text-sm text-primary hover:text-primary/80"
 >
 ...
 >
 {(unreadOverrides[conv.id] ?? conv.unreadCount) > 0 && (
 <span className="absolute top-2 left-2 min-w-[20px] h-5 px-1.5 bg-[var(--destructive)] text-[var(--primary-foreground)] text-xs font-bold rounded-full flex items-center justify-center">
 ...
 {/* Delete Confirmation */}
 {deleteTargetId && (
 <div className="absolute inset-0 bg-[var(--text-primary)]/[0.3] flex items-center justify-center z-50">
...
function MessageBubble({
 ...
 if (mimeType.includes('pdf')) return <FileText className="w-5 h-5 !text-[var(--destructive)]" />;
 ...
 {isOwn && (
 <div className="w-10 h-10 bg-[var(--primary)]/[0.1] rounded-full flex items-center justify-center flex-shrink-0">
 ...
 {msg.status === 'FAILED' && (
 <button onClick={() => onRetry(msg.id)} className="text-xs text-[var(--destructive)] underline">
 فشل - إعادة
 </button>
 )}
 {!isOwn && (
...
const DiscussionsView = () => {
 ...
 {detailError && (
 <div className="p-4 text-center">
 <p className="text-[var(--destructive)] mb-2">{detailError}</p>
 <button
 ...
 className="px-3 py-1 text-xs rounded-lg border border-red-300 text-[var(--destructive)] hover:bg-[var(--destructive)]/[0.08]"
 >
 حذف
 </button>
 </div>
 ...
 <span className="px-2 py-1 bg-[var(--destructive)]/[0.1] text-[var(--destructive)] text-xs rounded-full">
 فشل الإرسال
 </span>
 )}
 ...
 className="text-xs text-[var(--primary)] hover:text-[var(--primary)]/[0.8] disabled:opacity-50"
 >
 ...
 {replyErrors[discussion.id] && <p className="text-[var(--destructive)] text-sm">{replyErrors[discussion.id]}</p>}
 </div>
 )}
 </div>
 )}
 ...
 className="px-4 py-2 text-sm text-[var(--destructive)] hover:text-[var(--destructive)] border border-[var(--destructive)]/[0.3] rounded-lg"
 >
 حذف
 </button>
 </div>
 ...
 {deleteConfirmId && (
 <div className="fixed inset-0 bg-[var(--text-primary)]/[0.5] flex items-center justify-center z-50">
 ...
 disabled={detailMutating}
 className="px-4 py-2 bg-[var(--destructive)] text-[var(--primary-foreground)] rounded-lg hover:bg-[var(--destructive)]/90 disabled:opacity-50"
 >
 حذف
 </button>
 <button
 ...
 return <FileSpreadsheet className="w-8 h-8 text-[var(--primary)]" />;
 }
 ...
 <div className="p-4 bg-[var(--destructive)]/[0.08] border border-[var(--destructive)]/[0.3] rounded-xl flex items-center justify-between">
 ...
 switch (priority) {
 case 'high': return 'text-[var(--destructive)]';
 ...
 <div key={idx} className="flex items-center justify-between p-4 bg-[var(--destructive)]/[0.08] rounded-lg">
 <div>
 <p className="font-medium">{item.project}</p>
 <p className="text-sm text-muted-foreground mt-1">معين لـ: {item.assignee}</p>
 </div>
 <div className="text-left">
 <p className="text-sm font-medium text-[var(--destructive)]">تأخير: {item.overdue}</p>
 ...
 'bg-[var(--primary)]/[0.1] text-foreground'
 }`}>
 {row['9-12']}
 </div>
 </td>
 ...
 <div className="bg-card rounded-xl p-4 border border-border text-center">
 <Paperclip className="w-6 h-6 text-[var(--primary)] mx-auto mb-2" />
 ...
 <div className="bg-card rounded-xl p-4 border border-border text-center">
 <Edit className="w-6 h-6 text-[var(--warning)] mx-auto mb-2" />
 <p className="text-xl font-bold">15</p>
 <p className="text-xs text-muted-foreground">تعديل</p>
 </div>
 <div className="bg-card rounded-xl p-4 border border-border text-center">
 <CheckCircle2 className="w-6 h-6 text-[var(--primary)] mx-auto mb-2" />
