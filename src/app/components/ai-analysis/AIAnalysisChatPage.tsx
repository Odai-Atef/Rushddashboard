...
export function AIAnalysisChatPage() {
 ...
 switch (status) {
 case 'COMPLETED': return 'bg-[var(--primary)]/[0.2] text-[var(--primary)]';
 case 'RUNNING': return 'bg-[var(--primary)]/20 text-[var(--secondary)]';
 case 'FAILED': return 'bg-[var(--destructive)]/20 text-[var(--destructive)]';
 case 'PENDING': return 'bg-secondary0/10 text-muted-foreground';
 ...
 step.status === 'completed' ? 'bg-[var(--primary)]/[0.4]' : 'bg-muted'
 )}
 />
 )}
 </div>
 ...
 >
 {isUrgent && <AlertTriangle className="w-4 h-4 text-[var(--destructive)]" />}
 {isOpportunity && <Target className="w-4 h-4 text-[var(--primary)]" />}
 {!isUrgent && !isOpportunity && <Lightbulb className="w-4 h-4 text-[var(--secondary)]" />}
 </div>
 <div className="flex-1">
