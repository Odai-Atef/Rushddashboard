...
export function ProjectCreatePage() {
  ...
  const visibleFundingAreas = isEntityManager
    ? fundingAreas.filter((area) => {
        if (allowedFundingAreaIds?.size > 0 && allowedFundingAreaIds.has(area.id)) return true;
        if (allowedFundingAreaNames?.size > 0 && allowedFundingAreaNames.has(normalizeForMatch(area.name))) return true;
        // Fallback: if profile loaded but produced no matches, allow all so the user isn't blocked
        if (!allowedFundingAreaIds && !allowedFundingAreaNames) return true;
        return false;
      })
    : fundingAreas;
