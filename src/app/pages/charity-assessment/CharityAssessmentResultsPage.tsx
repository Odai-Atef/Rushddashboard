...
function getReadinessLevel(score: number) {
  ...
  if (score >= 55) return { label: 'متوسط', color: 'text-[var(--warning)]', bg: 'bg-[var(--warning)]' };
  return { label: 'يحتاج تحسين', color: 'text-[var(--destructive)]', bg: 'bg-[var(--destructive)]' };
}
...
function getSeverityClasses(severity: string) {
  switch (severity) {
    case 'critical':
      return {
        wrapper: 'bg-[var(--destructive)]/5 border-[var(--destructive)]/[0.2]',
        badge: 'text-[var(--destructive)]',
        label: 'حرج',
      };
    case 'high':
      return {
        wrapper: 'bg-[var(--warning)]/5 border-[var(--warning)]/[0.2]',
        badge: 'text-[var(--warning)]',
        label: 'عالي',
      };
    default:
      return {
        wrapper: 'bg-muted/[0.08] border-[var(--border)]',
        badge: 'text-[var(--muted-foreground)]',
        label: 'متوسط',
      };
  }
}
...
export function CharityAssessmentResultsPage() {
  ...
  if (error) {
    return (
      <div className="min-h-full bg-background flex items-center justify-center p-8">
        <div className="bg-card border border-border rounded-xl p-[var(--spacing-card-padding)] text-center max-w-md">
          <AlertTriangle className="w-12 h-12 text-[var(--destructive)] mx-auto mb-4" />
          <h2 className="text-xl font-semibold mb-2 text-card-foreground">تعذر تحميل النتائج</h2>
    ...
    return (
      <div className="min-h-full bg-background flex items-center justify-center p-8">
        <div className="bg-card border border-border rounded-xl p-[var(--spacing-card-padding)] text-center max-w-md">
          ...
          <h2 className="text-xl font-semibold mb-2 text-card-foreground">لا توجد نتائج</h2>
  ...
  const benchmarkData = benchmarks
    ? [
        { name: 'منظمتك', value: benchmarks.yourScore, color: 'var(--chart-2)' },
        { name: 'متوسط القطاع', value: benchmarks.sectorAverage, color: 'var(--chart-3)' },
        { name: 'أفضل ممارسة', value: benchmarks.topPerformer, color: 'var(--chart-1)' },
      ]
    : [
        { name: 'منظمتك', value: overallScore, color: 'var(--chart-2)' },
        { name: 'متوسط القطاع', value: 68, color: 'var(--chart-3)' },
        { name: 'أفضل ممارسة', value: 85, color: 'var(--chart-1)' },
      ];
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
          <div className="flex items-start justify-between mb-6">
            ...
            <div className="flex gap-2 flex-wrap items-center justify-end">
              <button
                ...
                className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-[var(--card)]/10 hover:bg-[var(--card)]/20 rounded-lg transition-colors disabled:opacity-60 disabled:cursor-not-allowed text-sm sm:text-base text-[var(--primary-foreground)]"
              >
                {isExporting ? (
              ...
              <button
                ...
                className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-[var(--card)]/10 hover:bg-[var(--card)]/20 rounded-lg transition-colors text-sm sm:text-base text-[var(--primary-foreground)]"
              >
                <RefreshCw className="w-4 h-4" />
                إعادة التقييم
              </button>
              {documentsMissing && data.qualificationStatus?.toUpperCase() !== 'NOT_QUALIFIED' && (
                <button
                  ...
                  className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-[var(--warning)] hover:bg-[var(--warning)]/90 text-[var(--primary-foreground)] rounded-lg transition-colors animate-pulse text-sm sm:text-base"
                >
                  أكمل ملف جهتك
                </button>
              )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-[var(--spacing-grid-gap)]">
            <div className="bg-[var(--card)]/10 backdrop-blur rounded-xl p-4 sm:p-[var(--spacing-card-padding)]">
              <p className="text-[var(--primary-foreground)]/80 mb-2">درجة الجاهزية الإجمالية</p>
            ...
            <div className="bg-[var(--card)]/10 backdrop-blur rounded-xl p-4 sm:p-[var(--spacing-card-padding)] md:col-span-2">
              <p className="text-[var(--primary-foreground)]/80 mb-2">التقييم العام</p>
      ...
      <div ref={reportRef} data-report-root className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        <div data-report-section className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-[var(--spacing-grid-gap)] mb-6 sm:mb-8">
          ...
          <div className="lg:col-span-2 bg-card border border-border rounded-xl p-4 sm:p-[var(--spacing-card-padding)]">
            <h2 className="text-xl font-semibold mb-6 text-card-foreground">نظرة شاملة على الأداء</h2>
            {radarData.length > 0 ? (
              <ResponsiveContainer width="100%" height={250} minHeight={250}>
                <RadarChart data={radarData}>
                  <PolarGrid stroke="var(--border)" />
                  <PolarAngleAxis dataKey="category" tick={{ fontSize: 12, fill: 'var(--muted-foreground)' }} />
                  <PolarRadiusAxis
                    angle={90}
                  ...
                  <Tooltip
                    ...
                    contentStyle={{ direction: 'rtl', textAlign: 'right', backgroundColor: 'var(--card)', border: '1px solid var(--border)', borderRadius: 'var(--radius)' }}
                  />
                </RadarChart>
              </ResponsiveContainer>
            ) : (
          ...
          <div className="space-y-4 sm:space-y-[var(--spacing-section-gap)]">
            <button
              ...
              className="bg-card border border-border/80 rounded-2xl p-4 sm:p-[var(--spacing-card-padding)] text-right w-full shadow-sm transition-all duration-200 hover:shadow-md cursor-pointer flex flex-col justify-between"
            >
              <div className="flex items-center justify-between mb-3 sm:mb-5">
                <div className="p-2 rounded-xl bg-[var(--warning)]/[0.1]">
                  <Star className="w-6 h-6 text-[var(--warning)]" />
                </div>
                <Award className="w-6 h-6 text-[var(--secondary)]" />
              </div>
              <p className="text-2xl sm:text-3xl font-bold text-card-foreground mb-1 tracking-tight">{strengths.length}</p>
            ...
            >
              <div className="flex items-center justify-between mb-3 sm:mb-5">
                <div className="p-2 rounded-xl bg-[var(--destructive)]/[0.1]">
                  <AlertTriangle className="w-6 h-6 text-[var(--destructive)]" />
                </div>
                <Target className="w-6 h-6 text-[var(--destructive)]" />
              </div>
              <p className="text-2xl sm:text-3xl font-bold text-card-foreground mb-1 tracking-tight">{weaknesses.length}</p>
            ...
            >
              <div className="flex items-center justify-between mb-3 sm:mb-5">
                <div className="p-2 rounded-xl bg-[var(--accent)]/[0.1]">
                  <Lightbulb className="w-6 h-6 text-[var(--accent)]" />
                </div>
                <Sparkles className="w-6 h-6 text-[var(--secondary)]" />
              </div>
              <p className="text-2xl sm:text-3xl font-bold text-card-foreground mb-1 tracking-tight">{recommendations.length}</p>
        ...
        <div data-report-section className="bg-card border border-border rounded-xl p-4 sm:p-[var(--spacing-card-padding)] mb-6 sm:mb-8">
          <h2 className="text-xl font-semibold mb-6 text-card-foreground">المقارنة المعيارية</h2>
        ...
        <div id="strengths-section" data-report-section className="bg-card border border-border rounded-xl p-4 sm:p-[var(--spacing-card-padding)] mb-6 sm:mb-8">
          <div className="flex items-center gap-[var(--spacing-small-gap)] mb-6">
            <Star className="w-6 h-6 text-[var(--warning)]" />
            <h2 className="text-xl font-semibold text-card-foreground">نقاط القوة الرئيسية</h2>
          </div>
                ...
                return (
                  <div key={index} className="bg-[var(--primary)]/5 border border-[var(--primary)]/[0.2] rounded-lg p-[var(--spacing-card-padding)]">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <h3 className="font-medium text-sm sm:text-base mb-1 text-card-foreground">{title}</h3>
        ...
        <div id="gaps-section" data-report-section className="bg-card border border-border rounded-xl p-4 sm:p-[var(--spacing-card-padding)] mb-6 sm:mb-8">
          <div className="flex items-center gap-[var(--spacing-small-gap)] mb-6">
            ...
            <h2 className="text-xl font-semibold text-card-foreground">تحليل الفجوات</h2>
          </div>
          {llmGapAnalysis ? (
                      ...
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1">
                          <div className="flex items-center gap-[var(--spacing-small-gap)] mb-2">
                            <h3 className="font-medium text-sm sm:text-base text-card-foreground">{gap.area}</h3>
                            <span className={`px-2 py-0.5 rounded-lg text-xs ${severityClasses.badge} bg-card border border-border`}>
                          ...
                          {gap.recommendation && (
                            <div className="flex items-start gap-2 bg-muted rounded-lg p-3 sm:p-[var(--spacing-card-padding)]">
                              <Lightbulb className="w-4 h-4 text-[var(--secondary)] flex-shrink-0 mt-0.5" />
                              <p className="text-xs sm:text-sm text-card-foreground">{gap.recommendation}</p>
                            </div>
                          )}
                        </div>
                      </div>
        ...
        {progressData && progressData.length > 0 && (
          <div data-report-section className="bg-card border border-border rounded-xl p-4 sm:p-[var(--spacing-card-padding)] mb-6 sm:mb-8">
            <h2 className="text-xl font-semibold mb-6 text-card-foreground">تتبع التقدم</h2>
            <ResponsiveContainer width="100%" height={250} minHeight={200}>
