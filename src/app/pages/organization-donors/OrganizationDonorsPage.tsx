    ...
    <div className="space-y-4 sm:space-y-5" dir="rtl">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div className="flex items-center gap-[var(--spacing-small-gap)]">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[var(--secondary)] to-[var(--chart-3)] flex items-center justify-center text-[var(--primary-foreground)]">
            <Building2 className="w-5 h-5" />
          </div>
          <div>
            <h1 className="text-lg sm:text-xl font-bold">الجهات المانحة</h1>
      ...
      {/* Error */}
      {error && (
        <div className="p-4 sm:p-[var(--spacing-card-padding)] rounded-xl bg-[var(--destructive)]/[0.08] border border-[var(--destructive)]/[0.3] text-[var(--destructive)] text-sm text-right">
          {error}
          <button
            onClick={refetch}
            className="mr-3 text-sm text-[var(--destructive)] hover:text-[var(--destructive)]/80 underline"
          >
            إعادة المحاولة
          </button>
        </div>
