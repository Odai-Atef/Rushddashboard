 ...
 *
 * Fetches and manages paginated project conversations.
 */
...
export interface UseProjectConversationsReturn extends ConversationsState {
...
export function useProjectConversations(
  ...
  const load = useCallback(
    async (filters: ConversationFilters, resetPage: boolean = false) => {
      if (!projectId) return;
      ...
      const requestFilters: ConversationFilters = {
        ...filters,
      ...
      try {
        const response: ApiResponse<PaginatedResponse<Conversation>> =
          await collaborationService.getProjectConversations(projectId, requestFilters, {
            signal: controller.signal,
          });

        if (!isMountedRef.current) return;

        const { data: conversations, total, page, limit, totalPages } = response.data;

        setState((prev) => ({
          ...prev,
          conversations:
            page === DEFAULT_PAGE ? conversations : [...prev.conversations, ...conversations],
