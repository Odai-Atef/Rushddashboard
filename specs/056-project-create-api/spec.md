...
**Feature Branch**: `[056-project-create-api]`  
**Created**: 2026-06-21  
**Status**: Draft  
...
- How does the system behave if the API returns a 201 response without a created project identifier?
...
- What happens when the manager or organization selection lists come from a separate API and are empty?
...
- **FR-002**: The form MUST send a request to `POST /api/v1/projects` with a JSON body matching the API contract when the user confirms project creation.
...
- The `/api/v1/projects` endpoint accepts `POST` requests and returns the created project object, possibly including an `id` field not specified in the provided contract.
